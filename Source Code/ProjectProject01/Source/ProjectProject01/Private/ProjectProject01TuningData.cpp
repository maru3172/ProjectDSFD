...
{
	FPlayerTuningRow ValidPlayerRow;
	FString Error;
	TestTrue(TEXT("Default player tuning is accepted"), ValidPlayerRow.IsValidForApplication(Error));

	FPlayerTuningRow InvalidPlayerRow = ValidPlayerRow;
...
namespace ProjectProject01Tuning
{
	...
	const TCHAR* PlayerTablePath = TEXT("/Game/MyProject/Data/DT_PlayerTuning.DT_PlayerTuning");
...
bool FPlayerTuningRow::IsValidForApplication(FString& OutError) const
{
	...
	{
		OutError = TEXT("DT_PlayerTuning Default row must contain finite, non-negative numeric values.");
		return false;
	}
...
{
	if (!FMath::IsFinite(MannequinWalkSpeed) || !FMath::IsFinite(DirectChaseRadius) ||
		!FMath::IsFinite(RoamingOuterRadius) || !FMath::IsFinite(PostPossessionCommandDurationSeconds) ||
		...
		!FMath::IsFinite(VisionFovMarginMultiplier) || MannequinWalkSpeed < 0.0f ||
		DirectChaseRadius < 0.0f || RoamingOuterRadius < 0.0f || PostPossessionCommandDurationSeconds < 0.0f ||
		DirectChaseHalfAngleDegrees < 0.0f || DirectChaseSectorRadius < 0.0f ||
		MannequinGatherRadius < 0.0f || RequiredMannequinCount < 0 ||
...
{
	PlayerTuningTable = LoadObject<UDataTable>(nullptr, ProjectProject01Tuning::PlayerTablePath);
	...
	bHasValidPlayerTuning = false;
	bHasValidMannequinTuning = false;
	bHasValidHelperTuning = false;

	if (!IsValid(PlayerTuningTable) || !IsValid(MannequinTuningTable) || !IsValid(HelperTuningTable))
	{
		OutError = TEXT("Player, AI, and Helper tuning DataTables are required. Use Tools > ProjectProject01 Tuning > Reimport and Validate Tuning DataTables first.");
		return false;
	}

	const FPlayerTuningRow* PlayerRow = PlayerTuningTable->FindRow<FPlayerTuningRow>(
	...
	const FHelperTuningRow* HelperRow = HelperTuningTable->FindRow<FHelperTuningRow>(
		ProjectProject01Tuning::DefaultRowName, TEXT("ProjectProject01 tuning validation"), false);
	if (PlayerRow == nullptr || MannequinRow == nullptr || HelperRow == nullptr)
	{
		OutError = TEXT("All tuning DataTables must contain a row named Default.");
		return false;
	}

	if (!PlayerRow->IsValidForApplication(OutError) || !MannequinRow->IsValidForApplication(OutError) ||
		!HelperRow->IsValidForApplication(OutError))
	{
		return false;
	}

	CachedPlayerTuning = *PlayerRow;
	CachedMannequinTuning = *MannequinRow;
	CachedHelperTuning = *HelperRow;
	bHasValidPlayerTuning = true;
...
bool UProjectProject01TuningSubsystem::ApplyToCurrentWorld(FString& OutError)
{
	...
	for (AActor* Actor : PlayerActors)
	{
		...
		{
			PlayerCharacter->ApplyPlayerTuning(CachedPlayerTuning);
...
bool UProjectProject01TuningSubsystem::GetPlayerTuning(FPlayerTuningRow& OutTuning) const
{
	if (!bHasValidPlayerTuning)
	{
		return false;
	}

	OutTuning = CachedPlayerTuning;
	return true;
}
